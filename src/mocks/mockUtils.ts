...
export const clearData = async (strapi: Strapi) => {
  ...
  const collectionTypeUids = ['api::restaurant.restaurant', 'api::cuisine.cuisine', 'api::dish.dish', 'api::address.address', 'plugin::users-permissions.user']
  const bulkClears = []
...
export const uploadFile = async (strapi: Strapi, {
  data,
  file,
}: UploadFile) => {
